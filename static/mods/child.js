layui.define(['form', 'layer', 'table', 'jquery', 'laydate'], function(exports) {
    ...
    form.on('submit(add-article)', function(data) {
        ...
        $.post('/Api/Article/add', data.field, function(res) {
    ...
    // 监听添加分类
    form.on('submit(add-cate)', function() {
        var index = layui.layer.open({
            title: "添加分类",
            type: 2,
            content: "/Admin/Category/manage",
    ...
    form.on('submit(edit-article)', function(data) {
        ...
        $.post('/Api/Article/edit', field, function(res) {
            if (typeof res == 'string') {
                res = eval('(' + res + ')');
            }
            layer.close(index);
    ...
    form.on('submit(article)', function(data) {
        ...
        if (event == 'search') {
            ...
            layer.confirm('确定删除这些文章么', function(index) {
                ...
                $.post('/Api/Article/del', {list: JSON.stringify(params)}, function(data) {
                    if (typeof data == 'string') {
                        data = eval('(' + data + ')');
                    }
                    if (data.code) {
